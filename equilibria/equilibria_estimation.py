...
def get_tress_in_cache(cache_dir):
    cache_acts = dict()
    ...
    for f in all_files:
        ...
        if precision is not None:
            time_ts = round(float(time_ts), precision)
        else:
            time_ts = float(time_ts)
        if time_ts in cache_acts:
            cache_acts[time_ts].append(track_id)
        else:
            cache_acts[time_ts] = [track_id]
    return cache_acts
